...
int isa_exec_once(Decode *s) {
     ...
     * LB/LH sign-extend the fetched byte/halfword up to 32 bits;
     * LBU/LHU zero-extend (paddr_read already returns a zero-padded
     * word_t, so no extra cast is needed). */
    INSTPAT("??????? ????? ????? 000 ????? 0000011", I,
            R(rd) = SEXT(paddr_read(src1 + imm, 1),  8));   /* LB  */
    INSTPAT("??????? ????? ????? 001 ????? 0000011", I,
            R(rd) = SEXT(paddr_read(src1 + imm, 2), 16));   /* LH  */
    INSTPAT("??????? ????? ????? 010 ????? 0000011", I,
            R(rd) = paddr_read(src1 + imm, 4));             /* LW  */
    INSTPAT("??????? ????? ????? 100 ????? 0000011", I,
            R(rd) = paddr_read(src1 + imm, 1));             /* LBU */
    INSTPAT("??????? ????? ????? 101 ????? 0000011", I,
            R(rd) = paddr_read(src1 + imm, 2));             /* LHU */

    /* --- stores ---------------------------------------------------- *
     * paddr_write masks to the low `len` bytes of `data` by memcpy. */
    INSTPAT("??????? ????? ????? 000 ????? 0100011", S,
            paddr_write(src1 + imm, 1, src2));              /* SB  */
    INSTPAT("??????? ????? ????? 001 ????? 0100011", S,
            paddr_write(src1 + imm, 2, src2));              /* SH  */
    INSTPAT("??????? ????? ????? 010 ????? 0100011", S,
            paddr_write(src1 + imm, 4, src2));              /* SW  */

    /* --- FENCE / FENCE.I — the emulator is sequentially consistent
     * and has a single-word icache-equivalent (re-fetch every step),
     * so both are no-ops here. */
    INSTPAT("??????? ????? ????? 000 ????? 0001111", I,
            /* FENCE   — NOP */ (void)0);
    INSTPAT("??????? ????? ????? 001 ????? 0001111", I,
            /* FENCE.I — NOP */ (void)0);
